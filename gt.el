...
(defun gt--quit-graphite ()
  "Kill the graphite buffer and exit."
  (interactive)
  (kill-buffer "*graphite*"))

(defun gt--graphite-buffer-exists-p ()
  "Visibility predicate."
  (not (equal (get-buffer "*graphite*") nil)))

(transient-define-suffix gt--graphite-clear-buffer (&optional buffer)
...
(transient-define-suffix gt--create (the-prefix-arg)
  ...
  (let* ((args (transient-args (oref transient-current-prefix command)))
    ...
    (when (gt--graphite-buffer-exists-p)
      (gt--graphite-clear-buffer))
